...
- Claude credential access reuses only an already-present access token from provider-owned env/native-file state; on Windows it also rediscovers provider-owned WSL `~/.claude/.credentials.json` files through `\\wsl$` and may read Windows Credential Manager. If a stored access token is rejected and unchanged on re-read, Token Lens now performs a bounded bare `claude` startup (60 s hard timeout, 1 s credential polling), discards all TUI output, lets Claude Code own any refresh/write, then retries `/api/oauth/usage` once; Token Lens never reads/redeems a Claude refresh token or writes credentials itself;
