...
- stats polling is visibility-aware at 30 seconds, while the compatibility layer caches Today (30s), Month (2m), derived ranges (1m), All Time (5m), and quota (5m); non-forced duplicate period/quota requests share the same in-flight load, while manual refresh bypasses caches and intentionally starts fresh reads;
...
1. validate the hardened Claude recovery path on the Claude-CLI machine: after a 401, confirm 30-second recovery polls do not generate repeated `/api/oauth/usage` 401/429 incidents while the credential is unchanged, and confirm a provider-owned credential rotation is consumed without waiting for the five-minute failure cooldown;
...
6. validate Gemini provider-owned CLI recovery again, including a startup that exceeds two minutes but completes within the 240-second bound.
