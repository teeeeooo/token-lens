...
Model and Session views share a compact provider-filter toolbar that is structurally outside the scrollable row list. Opening the filter menu is an overlay operation and must not reflow or reset the list. Those long-list views expose only a narrow, low-contrast scrollbar for position awareness; other surfaces may retain their existing hidden-scrollbar policy.
...
Floating Bubble is enabled by default and its default content is the compact provider-limit display: provider icon plus remaining quota percentage. Retain only six v1-derived display modes in v2: `limitsAllSessions`, `icon`, `barsSession`, `barsWeekly`, `barsAllSessions`, and `bars`. Provider-limit display may persist an explicit subset of the supported providers with no two-provider cap; an empty selection retains the historical Auto behavior of the first two usable providers, while explicit selection renders every selected usable provider in stable provider order. This selection is scoped only to `limitsAllSessions`; `barsAllSessions` retains its first-two-provider semantics. The collapsed surface remains one line at 34 logical pixels high and may grow to 320 logical pixels wide for the four-provider case. Do not restore the token/cost text modes or the custom tray/bubble composer merely for settings parity.

The compact quota selector follows the explicit v2 provider order and preserves the proven v1 primary/secondary semantics: prefer session, then daily, weekly, billing, and finally an otherwise-classified metered quota when a provider such as Gemini exposes model buckets without a canonical cadence. Additional Codex lanes are not eligible for the compact primary selection. In Auto mode, two or more eligible providers show the first two providers' primary remaining percentages; one eligible provider may show its primary and secondary percentages. When the user explicitly selects providers, every selected usable provider is shown with its primary remaining percentage in stable provider order.
...
- The enrichment reads only Claude Code access-token state from provider-owned storage. On Windows that includes the native credential file, provider-owned WSL `~/.claude/.credentials.json` files discovered through `\\wsl$`, and Windows Credential Manager when applicable. It calls the official usage endpoint. If the credential is missing or remains rejected after re-read, Token Lens may schedule one bounded zero-inference Claude auth-touch in the background and use only provider-owned credential fingerprint change as the success signal. On Windows, the official CLI is launched as bare `claude` inside a hidden real Windows console (`CREATE_NEW_CONSOLE` + hidden startup window), with no synthetic TUI input; the process is assigned to a Job Object before it is resumed so the entire provider process tree can be terminated after success or timeout. This replaces the ineffective Windows ConPTY + `/status` path and matches the observed user-launched CMD behavior more closely. Live Windows recovery subsequently confirmed the production path end to end: after `/api/oauth/usage` rejected the old credential, hidden bare `claude` rotated the provider-owned credential within a few seconds and quota recovered without user input. Non-Windows platforms retain the existing PTY startup plus one `/status` touch. Provider output must not be parsed, stored, or logged. Windows executable resolution must prefer inherited PATH/CMD semantics before known-location fallbacks. A provider-level single-flight guard prevents duplicate CLI startups. While recovery is pending or cooling down, the separate 30-second provider-recovery poll is credential-discovery-only: Token Lens must not call the usage endpoint again with the same rejected credential. Expiry of the CLI retry cooldown permits another CLI auth-touch, not another provider API request; direct provider probing resumes only after provider-owned credential state changes. Token Lens never reads/redeems refresh tokens, writes/rotates credentials itself, or creates a login/account-management flow.
- CLI credential-recovery lifecycle records use `CREDENTIAL_RECOVERY` / `credential_recovery` as their top-level log identity; the originating provider failure is preserved separately as `triggerCode` / `triggerStage`, so a successful refresh is not mislabeled as a new HTTP 401 or credential-discovery failure.
- Claude OAuth `429` handling is provider-protective rather than literal-header-only: provider `Retry-After` remains observable separately, while Token Lens applies a sticky in-memory consecutive-rate-limit cooldown floor of 5/15/30/60 minutes (bounded at 60 minutes). The absolute deadline derived from the provider-reported `Retry-After` is persisted separately in `provider-rate-limits.json`, so restarting Token Lens does not re-probe during an active server-issued cooldown. Adaptive streak/floor state is intentionally process-local, and persisted cooldown state is cleared only after an actual non-429 provider response.
...
- reuse only Gemini CLI-owned OAuth credential state. Support the current Gemini CLI secure-storage contract (`gemini-cli-oauth` / `main-account`), including Windows Credential Manager and Gemini CLI's encrypted `~/.gemini/gemini-credentials.json` file fallback, plus the older `~/.gemini/oauth_creds.json` migration source. An access token with unknown local expiry may be tried read-only and accepted only if the provider API accepts it. When the credential is missing, expired, or rejected, Token Lens may schedule one bounded zero-inference auth-touch in the background: run the official `gemini --list-sessions -e none --skip-trust` command from an isolated process-scoped temporary working directory, watch only for provider-owned credential fingerprint change for up to 60 seconds, and never send a model prompt or parse/store command output. Gemini CLI owns refresh/persistence and duplicate startups are suppressed. On Windows, the command runs inside a hidden real Windows console rather than ConPTY; the suspended root process is assigned to a Job Object before resume so success/timeout cleanup covers the full provider process tree. This hosting model is backed by live A/B evidence on an expired credential: the hidden real-console path rotated `oauth_creds.json` and extended expiry while the prior production ConPTY path repeatedly timed out. Windows launch resolution mirrors an ordinary CMD invocation: explicit override first, then the inherited PATH/PATHEXT `gemini` command, with `%APPDATA%\npm\gemini.cmd` as a fallback rather than a higher-priority shim. Diagnostics may expose only a coarse launch-source label, never a provider executable path or command output. While recovery is pending or cooling down, the separate 30-second provider-recovery poll reads only Gemini credential state and must not call `loadCodeAssist` / `retrieveUserQuota` again with the same rejected credential. Expiry of the CLI retry cooldown permits another CLI auth-touch, not another provider API request; a changed, currently usable provider-owned credential may be probed immediately. Stale last-good quota is presentation-only and must never satisfy a fresh-quota early return or refresh its own capture timestamp. When multiple provider-owned Windows credential stores exist, keep their priority order but select the first currently usable access-token snapshot so an expired higher-priority store cannot shadow a later provider/SSO update. Gemini 429 handling uses the same sticky in-memory 5/15/30/60-minute protective floor as Claude while preserving provider `Retry-After` separately. Its provider-reported Retry-After deadline is persisted in the same internal `provider-rate-limits.json` file and is cleared only after an actual non-429 provider response, so app restart cannot bypass an active server-issued cooldown. Token Lens must not read/redeem refresh tokens, write credentials itself, call `onboardUser`, or create/attach a Google Cloud project.
...
First launch follows a **progressive bootstrap** contract: Today usage is the first-render critical path; quota, Month, and All Time must not block the first usable dashboard render. Quota updates Limits when it arrives, while Month and All Time preload through the same cache/in-flight deduplication used by on-demand period requests. The full quota pipeline keeps its normal five-minute cache cadence but is split into `tokScale → parallel(Codex, Claude, Gemini, Antigravity) → provider-scoped merge`: each enrichment receives the same normalized tokScale base report, runs independently, and only its own provider row may be merged back. This preserves tokScale/provider authority and provider-specific account/workspace guards while preventing one enrichment result from overwriting another. If Claude or Gemini is waiting for provider-owned CLI credential recovery, a separate 30-second recovery command rechecks only those recovering provider adapters against the cached full report; stale last-good rows are never treated as authoritative input to that recheck, it must not rerun tokScale, Codex, or Antigravity, and provider-only recovery must not postpone the next five-minute full refresh. A normal full refresh may still compose the complete stats payload, but startup must not regress to waiting on all ranges or provider quota recovery before rendering Today.
...
Quota providers expose `status` (ready/stale/unavailable), `recoveryState` (idle/pending/cooldown), `lastSuccessAtMs`, `lastAttemptAtMs`, and `retryAtMs`. Diagnostic prose is explanatory only. Report assembly time is not a provider success time; recovery of one provider leaves other providers' timestamps untouched. Last-good fallback retains the original capture time and filters reset-expired windows. Claude/Gemini credential transitions conservatively invalidate private last-good snapshots; fingerprints never enter the renderer contract. CLI credential rotation keeps recovery pending until quota collection succeeds. Provider-only recovery treats its input row as a previous snapshot, never fresh tokScale input; last-good retention preserves valid canonical lanes while expiring reset-passed windows.

Dynamic derived results are held separately from fixed resource snapshots, with TTL deletion and a 64-entry LRU bound. Session metadata is requested only for the selected period, deduplicated, and collected in serial batches of at most 250 refs. A 4,096-entry, 60-second per-session cache stores only approved titles/project basenames; one batch failure does not discard other batches. These conservative bounds are tuning parameters, not claims of optimal live I/O performance. Rust's independent 5,000-ref and session-ID validation limits remain unchanged. Superseded metadata generations may neither publish nor overwrite newer cached metadata.
