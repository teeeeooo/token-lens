...
- Claude OAuth `429` handling is provider-protective rather than literal-header-only: provider `Retry-After` remains observable separately, while Token Lens applies a sticky consecutive-rate-limit cooldown floor of 5/15/30/60 minutes (bounded at 60 minutes). A larger provider cooldown, such as one hour, may not be shortened by a later tiny `Retry-After` until a non-rate-limited provider response resets the streak.
