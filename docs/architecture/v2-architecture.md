...
- The enrichment reads only Claude Code access-token state from provider-owned storage. On Windows that includes the native credential file, provider-owned WSL `~/.claude/.credentials.json` files discovered through `\\wsl$`, and Windows Credential Manager when applicable. It calls the official usage endpoint. If the credential is missing or remains rejected after re-read, Token Lens may schedule one bounded zero-inference Claude auth-touch in the background: launch the official Claude CLI through hidden PTY/ConPTY, send the TUI `/status` slash command once after startup, and use only provider-owned credential fingerprint change as the success signal. Provider output must not be parsed, stored, or logged. Windows executable resolution must prefer the inherited PATH/CMD command before known-location fallbacks so it matches ordinary user invocation as closely as practical. A provider-level single-flight guard prevents duplicate CLI startups. While recovery is pending or cooling down, the separate 30-second provider-recovery poll is credential-discovery-only: Token Lens must not call the usage endpoint again with the same rejected credential. Expiry of the CLI retry cooldown permits another CLI auth-touch, not another provider API request; direct provider probing resumes only after provider-owned credential state changes. Token Lens never reads/redeems refresh tokens, writes/rotates credentials itself, or creates a login/account-management flow. Current Windows runtime evidence records this hidden ConPTY `/status` path as ineffective on the tested machine: it times out without credential rotation, whereas a user-launched visible CMD running bare `claude` rotates the credential without additional input. Treat this as a known hosting-context limitation and investigate real-console hosting separately; do not infer that repeated `/status` attempts are equivalent to the successful user path.
