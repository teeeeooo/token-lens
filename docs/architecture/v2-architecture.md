...
- reuse only an already-present Gemini CLI-owned OAuth credential. Support the current Gemini CLI secure-storage contract (`gemini-cli-oauth` / `main-account`), including Windows Credential Manager and Gemini CLI's encrypted `~/.gemini/gemini-credentials.json` file fallback, plus the older `~/.gemini/oauth_creds.json` migration source. An access token with unknown local expiry may be tried read-only and accepted only if the provider API accepts it. When an existing stored credential is expired/rejected, Token Lens may start the official `gemini` executable with no prompt/slash command and wait for Gemini CLI itself to refresh its credential before one API retry. Token Lens must not read/redeem refresh tokens, write credentials itself, call `onboardUser`, or create/attach a Google Cloud project.
